...
if __name__ == '__main__':
    ...
    if len(args) == 2:
        ...
        df_term = pd.read_csv('../../IPC6_zh/test_hH.csv') # ADDED BY ZY FOR debugging
        termls = df_term['source_text'].to_list()

        fgold = open(gold, 'r')
        fpredictions = open(predictions, 'r')

        goldls = fgold.read().splitlines()
        predls = fpredictions.read().splitlines()


        if len(goldls)!=len(predls): sys.exit('ERROR: Number of lines in gold and output files differ')

        all_scores = []
        scores_names = ['MRR', 'MAP', 'P@1', 'P@3', 'P@5', 'P@10']
        for i in range(len(goldls)):
            ...
            print(precision_at_k(r,1,gold_hyps_n))
            avg_pat2.append(precision_at_k(r,3,gold_hyps_n))
            print(precision_at_k(r,3,gold_hyps_n))

            avg_pat3.append(precision_at_k(r,5,gold_hyps_n))
            print(precision_at_k(r,5,gold_hyps_n))

            avg_pat4.append(precision_at_k(r,10,gold_hyps_n))
